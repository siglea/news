...
| 2 标注 | [docs/steps/02-annotate.md](docs/steps/02-annotate.md) | **默认 `chat_json`**（`export-chat-bundle` → 对话 → `llm_annotations.json`）；高难度、严审稿用 **`terms_json`**（见 [docs/ANNOTATION.md](docs/ANNOTATION.md)） |
...
**对话标注与同位锚定**：[content/drafts/README.md](content/drafts/README.md)（主路径 `chat_json`；**`terms.json` 词表规则**供 `terms_json` 高难度稿使用）。
