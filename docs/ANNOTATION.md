...
2. **准备真源**：**`chat_json`**：`export-chat-bundle` → 对话产出 `llm_annotations.json`；**`keywords`**（仅编者单独要求时）：使用 [util/keyword_lexicon.py](../util/keyword_lexicon.py)（经 `annotate_lib` 暴露为 `KEYWORDS`）。  
