...
- **外源版权**：`meta.json` 中 `include_source_footer`、`footer_template`（`verbatim` / `derivative`）、`footer_derivative_mp_unknown`、`source_author_display`、`risk_blurb_secondary` 等；条文见 [EDITORIAL.md](../EDITORIAL.md)「外源素材与版权声明」等章节。
...
**`mingox build` 默认不校验句级密度**；密度、同位释义以 **[EDITORIAL.md](../EDITORIAL.md)**（「落地标准」等条）+ 人工为准。
