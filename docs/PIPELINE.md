...
### 2026-04-30（mx 加固 + 多 agent 协作 + 首篇实战）

- **mx 第一阶段加固**：一次性合入 9 条 PR 修复了 8 条问题清单 —— `.gitignore` 漏配、`.coze` 引用不存在脚本、首页孤儿稿、`npx edgeone@latest` 不 pin、缺 CI 入口、微信抽取黑名单全文匹配、220 字硬合并跨主题、`extract_ps` 静默丢段、整仓静态根目录暴露内部源码。**结论：基础设施加固先于业务工作，一次成型胜过零散修补。**
...
- **多 agent 并行 deploy 撞车**：实战首篇收尾时 @claude / @cursor 同时跑 `mingox deploy`，相差 28 秒，两份独立 deployment 都成功，但生产环境只跟最新一次。**虽然没坏事，是隐患**。**结论 → 串行 deploy 协议**：
  ...
  2. 执行中每 30~60s 报一次状态
  3. 完成后必回贴 `Deployment ID + 完整预览 URL（含 eo_token）+ eo_time`
  4. 其他 agent 看到 claim 后**不再并发**，除非明确接力或重试
- **EdgeOne CLI 副作用**：`mingox deploy` 跑完后 EdgeOne CLI 把 `dist/` move 到 `.edgeone/assets/`，覆盖了原本 tracked 的 `.edgeone/assets/*`（虽然 `.gitignore` 有 `.edgeone/*`，但已 tracked 文件不受 ignore 约束）。**结论：未来可加 PR 把 `.edgeone/assets/` 整体 `git rm --cached`，让 `.gitignore` 真正生效，避免每次 deploy 都让工作区出现 D/M 噪音。** （第二阶段候选）
- **首篇 throughput 基线**：从 URL 给到 deploy 成功 ~90 分钟，其中等人工提供 token ~70 分钟，纯流水线工作 ~20 分钟（acquire 1min + 清理/补 meta_description 3min + bundle 1min + subagent 标注 5min + build/validate <1min + commit/push/PR/review/merge 5min + deploy 1min）。**结论：在凭据齐全的稳态下，新稿端到端 ≤ 20 分钟可达。**
