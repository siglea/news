...
| **内容编辑 / 写稿** | [EDITORIAL.md](./EDITORIAL.md)（标题、首页列表、词汇密度、相邻块、外源版权）；[ANNOTATION.md](./ANNOTATION.md)（**默认 `chat_json`**，`chat_json` **不必 Cursor**，仅编者单独要求时改用 `keywords`）；单篇草稿 [content/drafts/README.md](../content/drafts/README.md) |
...
| [ANNOTATION.md](./ANNOTATION.md) | `chat_json` / `keywords`、真源、校验与 `validate` 分工；**非 Cursor 环境**如何用大模型产出 JSON |
...
| [util/README.md](../util/README.md) | 抓取、段落抽取、`annotate_lib`、`annotate_merge` |
