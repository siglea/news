...
| 文件 | 作用 |
|------|------|
| `mingox.py` | CLI：`init` / `acquire` / `build` / `export-chat-bundle` / `synth-lexicon-annotations` / `validate` / `serve` / `deploy` / `wechat` |
...
| `synth_llm_annotations_lexicon.py` | **非 chat_json 终稿**：keywords 式稀疏占位；与 `mingox wechat` / `article-profiles` **无关**。`chat_json` 须对话产出 JSON，见 [docs/EDITORIAL.md](../docs/EDITORIAL.md)。 |
...
- **`util/article-profiles.json`**：仅 **Legacy** `annotate-wechat-plain` 用；**新稿请用 `content/drafts/`，勿再扩张 profiles**。
